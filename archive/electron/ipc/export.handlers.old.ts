...
/**
 * Register IPC handlers for export operations
...
export function registerExportHandlers() {
  ...
  ipcMain.handle('export:selectDirectory', async () => {
    try {
      ...
      logger.error('export:selectDirectory failed', error as Error);
      throw error;
    }
  });
  ...
  ipcMain.handle('export:selectFile', async (event, { defaultName }) => {
    try {
      ...
      logger.error('export:selectFile failed', error as Error);
      throw error;
    }
  });
  ...
  ipcMain.handle('export:markdown', async (event, { noteIds, directory }) => {
    try {
      ...
      logger.error('export:markdown failed', error as Error);
      throw error;
    }
  });
  ...
  ipcMain.handle('export:json', async (event, { noteIds, filePath }) => {
    try {
      ...
      logger.error('export:json failed', error as Error);
      throw error;
    }
  });
